export function validateLayout(parsed) {
  ...
  if (!layout.conveyors?.length) errors.push("Add at least one green conveyor/intake spawn point.");
  if (!layout.bins?.length) errors.push("Add at least one orange bin/dropoff zone.");
  ...
  if (counts.wall + counts.shelf + counts.restricted > counts.floor * 1.7) {
